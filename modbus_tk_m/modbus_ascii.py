...
class RtuQuery(Query):
    ...
    def build_request(self, pdu, slave):
        ...
        data= ''
        hex_data={10:'A',11:'B',12:'C',13:'D',14:'E',15:'F'}       
        data1 = struct.pack(">B", self._request_address) + pdu
        for i in struct.unpack('>BBBBBB',data1):
        ...
        crc = '%x'%((0xff ^ int('%x' % sum(map(ord,data1)),16))+1)

        return (':'+data + crc.upper()+'\r\n')
    
    def parse_response(self, response):
        """Extract the pdu from the Modbus RTU response"""
       # response='\x01\x03\x04@\x80\x00\x008'
        ...
        print self._response_address,self._request_address
        ...
        response_caculate= '%x' % int(('%x'%((0xff ^ int('%2x' % sum(map(ord,response[0:-1])),16))+1))[-2:],16)
        
        print response_caculate

        crc  = '%x' % int(binascii.b2a_hex(response[-1:]),16)
        
        print crc,response_caculate
        
        if crc != response_caculate:
...
class RtuMaster(Master):
    ...
    def _recv(self, expected_length=-1):
        ...
        while read_bytes:
            ...
            if expected_length>=0 and len(response)>= 2*expected_length+1:
