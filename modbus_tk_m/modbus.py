...
class Master:
    ...
    @threadsafe_function
    def execute(
        self, slave, function_code, starting_address,
        quantity_of_x=0, output_value=0, data_format="", expected_length=-1,
        wait_for_response=True,  # 'wait_for_response' was added by zhen.zhang on 2012-08-08
        print_message=True      # 'print_message' was added by zhen.zhang on 2012-08-14
    ):
        """
       ...
       # print repr(pdu)
        ...
        if print_message:  # Added by zhen.zhang on 2012-08-14
            line = time.strftime('[%Y-%m-%d %H:%M:%S') + ('.%03d]' % (int(time.time() * 1000) % 1000)) + ' Tx:' + (' %02x' * len(request)) % struct.unpack('>' + 'B' * len(request), request)
            print line

        call_hooks("modbus.Master.after_send", (self))

        #if slave != 0:                            # commented by zhen.zhang on 2012-08-08
        if (slave != 0) and wait_for_response:     # added by zhen.zhang on 2012-08-08
            # receive the data from the slave
            
            response = self._recv(expected_length)
            print repr(response),1111
            ...
            if print_message:  # Added by zhen.zhang on 2012-08-14
                line = time.strftime('[%Y-%m-%d %H:%M:%S') + ('.%03d]' % (int(time.time() * 1000) % 1000)) + ' Rx:' + (' %02x' * len(response)) % struct.unpack('>' + 'B' * len(response), response)
                print line

            # extract the pdu part of the response
          
            response_pdu = query.parse_response(response)
            print response_pdu,1111111
            
           # response_pdu='\x03\x04\x40\x80\x00\x00'
           
            # analyze the received data
            (return_code, byte_2) = struct.unpack(">BB", response_pdu[0:2])
            
            print return_code, byte_2
            ...
            if return_code > 0x80:
                ...
                #y= struct.pack('f',result[0])
               # print type(result[0])
              
               # result=struct.unpack('f',struct.pack('hh',struct.unpack('hh',y)[1],struct.unpack('hh',y)[0]))
...
class ModbusBlock:
    ...
    def __init__(self, starting_address, size, block_name=''):  # 'block_name' was added by zhen.zhang on 2012-08-14
        """
        ...
        self.block_name = block_name   # Added by zhen.zhang on 2012-08-14
    ...
    def __getitem__(self, r):
        """"""
        call_hooks("modbus.ModbusBlock.getitem", (self, r, self.block_name)) # Added by zhen.zhang on 2012-08-15
    ...
    def __setitem__(self, r, v):
        ...
        call_hooks("modbus.ModbusBlock.setitem", (self, r, v, self.block_name)) # Added by zhen.zhang on 2012-08-14
...
class Slave:
    ...
    def add_block(self, block_name, block_type, starting_address, size):
        ...
        with self._data_lock: #thread-safe
            ...
            #self._memory[block_type].insert(index, ModbusBlock(starting_address, size))  # commented by zhen.zhang on 2012-08-08
            #new_block = ModbusBlock(starting_address, size)             # added by zhen.zhang on 2012-08-08
            new_block = ModbusBlock(starting_address, size, block_name)  # added by zhen.zhang on 2012-08-14
            self._memory[block_type].insert(index, new_block)            # added by zhen.zhang on 2012-08-08
            return new_block                                             # added by zhen.zhang on 2012-08-08
